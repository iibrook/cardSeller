...
import com.card.seller.domain.FieldType;

/**
 * orm属性过滤器，可以通过{@link PropertyFilters#build(String, String)}创建，使用他创建可以直接
 * 写入表达式即可，相关表达式样式查看{@link com.card.seller.dao.hibernate.CriterionBuilder}的实现类,如果直接创建查看{@link com.card.seller.dao.hibernate.CriterionBuilder}
 * 实现类的实际restrictionName值和{@link com.card.seller.domain.FieldType}枚举值,如果一个属性对比多个值可以使用逗号(,)分割
 * <p/>
 * <p>
 * 例子:
 * </p>
 * <code>PropertyFilter filter = new PropertyFilter("EQ",FieldType.S.getValue(),new String[]{"propertyName"},"a,b,c");<code>
 * <p>当使用以上filter到{@link com.card.seller.dao.hibernate.HibernateSupportDao#findUniqueByPropertyFilter(java.util.List)}是会产生以下sql</p>
 ...
 * @see com.card.seller.dao.hibernate.CriterionBuilder
 */
